...
function overlayHtml() {
  ...
  if (state.event) {
    ...
    return `<div class="overlay event-overlay"><div class="report event-report ${tone}"><div class="event-head"><div><span class="event-code">${eventCode}</span><div class="eyebrow">CITY REPORT · DAY ${String(state.day).padStart(2,'0')}</div></div><span class="event-required">DECISION REQUIRED</span></div><h2>${event.title}</h2><p class="event-text">${event.text}</p>${flight ? `<p class="flight-loss">离城 ${flight.count} 人 · 食物 −${flight.food} · 木材 −${flight.wood} · 煤炭 −${flight.coal}</p>` : ''}<div class="decision-list">${event.choices.map((c,i) => `<button class="choice" data-act="event" data-choice="${i}"><span class="choice-no">0${i + 1}</span><span class="choice-copy"><b>${c.label}</b><small>${c.consequence}</small></span><span class="choice-arrow" aria-hidden="true">›</span></button>`).join('')}</div></div></div>`;
  }
