...
function mapHtml() {
  ...
  const slots = rings.flatMap((count, ringIndex) => Array.from({ length: count }, (_, n) => {
    ...
    const homeCold = b?.type === 'house' && heat < -5;
    ...
    const status = riot.shutdown ? { cls: 'riot-shutdown', text: `停摆 ${riot.shutdownRemaining}h`, mark: '停' }
      : riot.strikeAbsent ? { cls: 'riot-strike', text: `${riot.strikeAbsent}人拒工 · ${riot.strikeRemaining}h`, mark: '拒' }
      : riot.blocked ? { cls: 'riot-blocked', text: `道路封锁 · ${riot.blockRemaining}h`, mark: '封' }
      : criticalCold ? { cls: 'freeze', text: '严重失温', mark: '冻' }
      : homeCold ? { cls: 'homecold', text: '住宅失温', mark: '冷' }
      : noCrew ? { cls: 'idle', text: '无人工作', mark: '空' }
      : lowCrew ? { cls: 'lowcrew', text: '人手不足', mark: '缺' } : null;
...
function staffPanel() {
  ...
  return { title: '人员调度', subtitle: `可用 ${availableWorkers(state)} / 健康劳动力 ${workforce(state)} / 已分配 ${assigned(state)}`, body: `<div class="stat-grid"><div class="stat">总人口<b>${state.population}</b></div><div class="stat">健康劳动力<b>${workforce(state)}</b></div><div class="stat">儿童<b>${state.children}</b></div><div class="stat">病患<b>${state.sick}</b></div></div><div class="section-label">工作岗位</div>${list.length ? list.map(b => {
    ...
    return `<div class="card ${riot.shutdown || riot.strikeAbsent || riot.blocked ? 'riot-row' : ''}"><div class="cardline"><div class="card-main"><strong>${BUILDINGS[b.type].name}</strong><small>${ringName(ringOf(b.i))} · ${b.workers}/${BUILDINGS[b.type].workers * b.level} 人${riotText}</small></div><div class="staff-control"><button class="secondary" data-act="staff" data-index="${b.i}" data-delta="-1" aria-label="减少${BUILDINGS[b.type].name}工人" ${riot.blocked || riot.strikeAbsent || riot.shutdown ? 'disabled' : ''}>−</button><button data-act="staff" data-index="${b.i}" data-delta="1" aria-label="增加${BUILDINGS[b.type].name}工人" ${riot.blocked || riot.strikeAbsent || riot.shutdown ? 'disabled' : ''}>+</button></div></div></div>`;
...
function render() {
  ...
  const housingSpare = Math.max(0, homes - state.population);
  const housingTight = homeless === 0 && housingSpare <= 3;
