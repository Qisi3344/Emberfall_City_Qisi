...
- 新局依次经过加载页 → 执政者命名 → 开场文案 → 选择微寒 / 极寒 → 询问是否看教程；姓名 2～12 字符，本地保存上次输入，浏览器生成稳定 playerId。
- 微寒保留原有数值；极寒采用更低储备、更快降温、扩圈煤耗与后期户外停产。旧存档按微寒继续，新局选定难度后本局不可切换。
- 固定 6 / 8 / 10 个建筑槽，建造、升级、拆除、派工。
- 煤、木、钢、食物、住房、疾病、希望与不满；发电机煤耗和超载。
- 工坊研究扩圈与供暖，法令、城市事件、Day 20 风暴、胜败结算。
- 低希望引发离城倾向、每日逃亡及大规模离城危机；高不满引发抗议、生产下降与暴乱最后通牒（微寒 48 小时，极寒 24 小时）。
- 浏览器本地自动存档与本机排名；结算和排名记录执政者姓名、playerId 与难度。
...
地图左上方“希望”下的 ⚙ 打开，打开期间游戏时间完全暂停：音量、雪花粒子、生存手册、重新开始本局（二次确认）与返回。AI / MCP 入口目前锁定。
...
设置页的接入指南暂未开放；开发者仍可按下述方式连接。`create_ruler` 可传 `difficulty: "mild"` 或 `"extreme"`，默认微寒。
